...
要求：
- summary：不超过 200 字的中文摘要，概括会话主线、当前状态与未决事项。journal 中存在大段高度重复的描述时，只用一句话概括（例如"助手对同一画面输出了大量重复描述"），不要逐条罗列、不要在 summary 里复述重复内容。
- pins：从 journal 中"逐字"摘出最重要的信息片段。必须是逐字复制粘贴的原文子串——一个字符、一个标点、一个空格都不能改，数字保持原格式（如 011202 不得写成中文数字），引号与大小写保持原样；宁缺毋滥，拿不准能否逐字时就不要这条 pin。优先级如下：
  1. 用户的纠正（包括对命名、事实、格式的纠正）；
  2. 专有名词（人名、品牌/型号、项目名、路径、术语）；
  3. 带单位的数字与具体数值（价格、日期、尺寸、距离等）；
  4. 助手作出的承诺或待办；
  5. 用户的称呼/语言等偏好。
- pins 最多 16 条，每条最多 256 字符，总共最多 320 token。优先保留纠正、当前约束和未完成承诺；重复观察、轮次计数、无变化描述和寒暄不要逐条摘录。没有重要事实时返回空数组 []——空数组永远是合法答案。
